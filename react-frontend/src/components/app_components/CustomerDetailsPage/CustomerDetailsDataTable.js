...
const CustomerDetailsDataTable = ({
  ...
  const pTemplate0 = (rowData, { rowIndex }) => <p>{rowData.customers}</p>;
  const pTemplate1 = (rowData, { rowIndex }) => <p>{rowData.customerEmail}</p>;
  const dropdownTemplate2 = (rowData, { rowIndex }) => (
    <p>
      {`Home, ${rowData.customerAddress?.city || ""}, ${rowData.customerAddress?.postalCode || ""}`}
    </p>
  );
  const pTemplate3 = (rowData, { rowIndex }) => (
    <p className="m-0 ml 5">{rowData.phoneNumber}</p>
  );
    ...
    <>
      ...
      >
        ...
        <Column
          ...
          header="Customers"
          body={pTemplate0}
          filter={selectedFilterFields.includes("customers")}
          hidden={selectedHideFields?.includes("customers")}
          sortable
          style={{ minWidth: "8rem" }}
        />
        <Column
