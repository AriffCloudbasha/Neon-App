...
const CategoryCreateDialogComponent = (props) => {
  ...
  useEffect(() => {
    ...
    if (!_.isEmpty(props?.entity)) {
      init = initilization({ ...props?.entity, ...init }, [isSale], setError);
    }
    set_entity({ ...init });
    setError({});
  }, [props.show]);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("category").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((res) => {
        setIsSale(
          res.data.map((e) => {
            return { name: e["discountedPrice"], value: e._id };
          }),
        );
      })
      .catch((error) => {
  ...
  const isSaleOptions = isSale.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
      ...
      >
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="isSale"
              value={_entity?.isSale?._id}
              optionLabel="name"
              optionValue="value"
              options={isSaleOptions}
              onChange={(e) => setValByKey("isSale", { _id: e.value })}
            />
          </span>
          <small className="p-error">
