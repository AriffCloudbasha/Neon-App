...
const ProductRatingDataTable = ({
  ...
  const pTemplate1 = (rowData, { rowIndex }) => (
    <p className="m-0 ml-4">{rowData.customerName}</p>
  );
  const ratingTemplate2 = (rowData, { rowIndex }) => (
    <Rating
      stars={5}
      style={{ width: "20rem" }}
      value={rowData.starRating}
      cancel={false}
    />
  );
    ...
    <>
      ...
      >
        ...
        <Column
          ...
          filter={selectedFilterFields.includes("starRating")}
          hidden={selectedHideFields?.includes("starRating")}
          sortable
          style={{ minWidth: "8rem" }}
        />
