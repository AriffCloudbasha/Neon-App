...
const CartDataTable = ({
  ...
  const currencyTemplate2 = (rowData, { rowIndex }) => (
    <p className="m-0 ml-1">MYR {Number(rowData.subtotal || 0).toFixed(2)}</p>
  );
  const currencyTemplate3 = (rowData, { rowIndex }) => (
    <p className="m-0 ml-1">
      {rowData.total != null ? `MYR ${rowData.total.toFixed(2)}` : ""}
    </p>
  );
  const currencyTemplate4 = (rowData, { rowIndex }) => (
    <p className="m-0 ml 2">
      {rowData.tax != null ? `MYR ${rowData.tax.toFixed(2)}` : ""}
    </p>
  );
    ...
    <>
      ...
      >
        ...
        <Column
          ...
          filter={selectedFilterFields.includes("tax")}
          hidden={selectedHideFields?.includes("tax")}
          sortable
          style={{ minWidth: "8rem" }}
        />
